...
Create a `.config` folder at you user folder, then add another folder named `fastfetch` so you can put in there `ascii.txt`, `config.jsonc` and `old-logo-ascii.txt`, then just create a profile with this command:
```powershell
